...
**Reading.** Identical three-regime shape to the L2 experiment, one level down: (1) **below ~24 KB** both kernels' regions fit in L1 and colocated = alone; (2) **at the 32 KB knee** the combined 4× footprint crosses the 128 KB L1 and colocated latency explodes to **>6× alone** (worse than serializing) as the two kernels evict each other; (3) **above ~56 KB** a single kernel alone already overflows L1, so colocation can't make locality any worse and the ratio falls back to ~1. Note the two kernels never share a warp scheduler here — the entire effect comes from the shared L1.
