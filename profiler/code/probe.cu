...
// ------------------------- kernels -------------------------
__global__ void k_sleep(long long it){ for(long long i=0;i<it;i++) asm volatile("nanosleep.u32 1000;"); }

__global__ void k_copy(const float* in, float* out, long long n, long long it){
...
__global__ void k_copy_tb(const float* in, float* out, long long npb, long long it, int region_bytes){
...
struct Workload {
    string name;
    int blocks, threads;
    function<void(cudaStream_t)> launch;   // one timed iteration
...
Workload make(const string& name){
    ...
    if(name=="sleep"){
        ...
        w.launch=[=](cudaStream_t s){ k_sleep<<<g_sms,768,0,s>>>(it); };
    } else if(name=="dram"){
        ...
        w.launch=[=](cudaStream_t s){ k_copy<<<g_sms,512,0,s>>>(in,out,n,it); };
    } else if(name=="l2"){
        ...
        w.launch=[=](cudaStream_t s){ k_copy_tb<<<g_sms,64,0,s>>>(in,out,npb,it,rb); };
    } else if(name=="fma"){
        ...
        w.launch=[=](cudaStream_t s){ k_fma32<<<g_sms,128,0,s>>>(a,b,c,it); };
    } else if(name=="fp64"){
        ...
        w.launch=[=](cudaStream_t s){ k_fma64<<<g_sms,128,0,s>>>(a,b,c,it); };
...
double timeAlone(Workload& w, int reps){
    ...
    w.launch(s); CK(cudaStreamSynchronize(s)); // warm up
    vector<double> lat;
    cudaEvent_t a,b; CK(cudaEventCreate(&a)); CK(cudaEventCreate(&b));
    for(int i=0;i<reps;i++){ CK(cudaEventRecord(a,s)); w.launch(s); CK(cudaEventRecord(b,s)); CK(cudaEventSynchronize(b));
...
double timeColoc(Workload& tgt, Workload& ant, int reps, double tgtAlone, double antAlone){
    ...
    for(int i=0;i<nBatch;i++) ant.launch(sb);            // queue antagonist work (do NOT wait)
    vector<double> lat;
    cudaEvent_t a,b; CK(cudaEventCreate(&a)); CK(cudaEventCreate(&b));
    for(int i=0;i<reps;i++){ CK(cudaEventRecord(a,sa)); tgt.launch(sa); CK(cudaEventRecord(b,sa)); CK(cudaEventSynchronize(b));
        float ms; CK(cudaEventElapsedTime(&ms,a,b)); lat.push_back(ms); }
...
int main(int argc,char** argv){
    ...
    if(cmd=="once"){   // exactly one launch, for clean NCU profiling (--launch-count 1)
        ...
        w.launch(s); CK(cudaStreamSynchronize(s)); return 0;
    }
    if(cmd=="coloc"){
